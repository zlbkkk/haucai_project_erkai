...
def load_debugtalk(project):
    ...
    # 获取当前项目的 debugtalk 代码
    try:
        current_code = models.Debugtalk.objects.get(project__id=project).code or ""
    except ObjectDoesNotExist:
        current_code = ""

    # 获取公共项目的 debugtalk 代码
    common_project_id = get_common_project_id()
    common_code = ""
    if common_project_id and common_project_id != project:
        try:
            common_code = models.Debugtalk.objects.get(project__id=common_project_id).code or ""
        except ObjectDoesNotExist:
            pass
    ...
    try:
        FileLoader.dump_python_file(file_path, merged_code)
