...
def get_common_project_id():
    """
    获取公共项目的ID
    如果公共项目不存在，返回None
    """
    try:
        common_project = models.Project.objects.get(name=COMMON_PROJECT_NAME)
        return common_project.id
    except ObjectDoesNotExist:
        return None

def get_project_filter_condition(project_id):
    """
    获取项目过滤条件，包含当前项目和公共项目
    ...
    Returns:
        Q对象，用于Django ORM查询
    """
    # 如果项目ID为空或无效，返回空的查询条件
    if project_id is None or project_id == "":
        return Q(pk__isnull=True)  # 返回空结果
    ...
    try:
        ...
        if common_project_id and common_project_id != project_id:
            # 返回当前项目或公共项目的数据
            return Q(project__id=project_id) | Q(project__id=common_project_id)
        else:
            # 如果没有公共项目或当前就是公共项目，只返回当前项目数据
            return Q(project__id=project_id)
    except Exception:
        # 如果出现任何异常，回退到原始逻辑
        return Q(project__id=project_id)
