...
def run_api(request):
    ...
    if name != "请选择":
        try:
            config_obj = models.Config.objects.get(name=name, project__id=api.project)
            try:
                config = eval(config_obj.body) if config_obj.body else {}
                if not isinstance(config, dict):
                    config = {}
            except (SyntaxError, ValueError, TypeError):
                logger.error(f"Failed to parse config body for config {config_obj.id}")
                config = {}
    ...
    summary = loader.debug_api(
        api.testcase, api.project, name=api.name, config=parse_host(host, config), user=request.user
    )
...
def run_api_pk(request, **kwargs):
    ...
    if name != "请选择":
        try:
            config_obj = models.Config.objects.get(name=name, project=api.project)
            config = eval(config_obj.body) if config_obj.body else {}
            if not isinstance(config, dict):
                config = {}
        except (ObjectDoesNotExist, SyntaxError, ValueError, TypeError):
            config = {}

    try:
        test_case = eval(api.body) if api.body else {}
...
def auto_run_api_pk(**kwargs):
    ...
    try:
        config_obj = models.Config.objects.get(name=config_name, project=api.project)
        config = eval(config_obj.body) if config_obj.body else {}
        if not isinstance(config, dict):
            config = {}
    except (ObjectDoesNotExist, SyntaxError, ValueError, TypeError):
...
def run_api_tree(request):
    ...
    if config != "请选择":
        try:
            config_obj = models.Config.objects.get(name=config, project__id=project)
            config = eval(config_obj.body) if config_obj.body else {}
            if not isinstance(config, dict):
                config = {}
        except (ObjectDoesNotExist, SyntaxError, ValueError, TypeError):
    ...
    for relation_id in relation:
        ...
        for content in api:
            try:
                api_data = eval(content["body"]) if content["body"] else {}
                if not isinstance(api_data, dict):
                    api_data = {}
            except (SyntaxError, ValueError, TypeError):
                api_data = {}
            test_case.append(parse_host(host, api_data))
...
def run_testsuite_pk(request, **kwargs):
    ...
    for content in test_list:
        try:
            body = eval(content["body"]) if content["body"] else {}
            if not isinstance(body, dict):
                body = {}
        except (SyntaxError, ValueError, TypeError):
            body = {}

        if body.get("request", {}).get("base_url"):
            try:
                config_obj = models.Config.objects.get(name=body["name"], project__id=project)
                config = eval(config_obj.body) if config_obj.body else {}
                if not isinstance(config, dict):
                    config = {}
            except (ObjectDoesNotExist, SyntaxError, ValueError, TypeError):
...
def run_multi_tests(request):
    ...
    if back_async:
        ...
        summary, _ = loader.debug_suite(
            test_sets, project, suite_list, config_list, save=True, user=request.user, report_name=report_name
        )
