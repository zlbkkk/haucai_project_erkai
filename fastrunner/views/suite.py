...
class TestCaseView(GenericViewSet):
    ...
    def get(self, request):
        ...
        if ser.is_valid():
            ...
            # update_time 降序排列，包含当前项目和公共项目的数据
            project_filter = get_project_filter_condition(project)
            queryset = self.get_queryset().filter(project_filter).order_by("-create_time")
    ...
    def patch(self, request, **kwargs):
        ...
        models.Case.objects.filter(id=pk).update(
            update_time=datetime.datetime.now(), updater=request.user.username, **request.data
        )
