...
from fastrunner.utils import day, prepare, response
...
class DebugTalkView(GenericViewSet):
    ...
    def debugtalk(self, request, **kwargs):
        """
        得到debugtalk code，包含当前项目和公共项目的代码
        """
        from fastrunner.utils.prepare import get_common_project_id

        pk = kwargs.pop("pk")
        try:
            # 获取当前项目的 debugtalk
            current_debugtalk = models.Debugtalk.objects.get(project__id=pk)
            current_code = current_debugtalk.code or ""

            # 获取公共项目的 debugtalk
            common_project_id = get_common_project_id()
            common_code = ""
            if common_project_id and common_project_id != int(pk):
                try:
                    common_debugtalk = models.Debugtalk.objects.get(project__id=common_project_id)
                    common_code = common_debugtalk.code or ""
                except ObjectDoesNotExist:
                    pass

            # 合并代码：公共项目的代码在前，当前项目的代码在后
            merged_code = ""
            if common_code:
                merged_code += "# ===== 公共项目自定义函数 =====\n"
                merged_code += common_code
                merged_code += "\n\n\n\n\n"

            if current_code:
                merged_code += "# ===== 当前项目自定义函数 =====\n"
                merged_code += current_code

            # 创建一个临时的响应数据，包含合并后的代码
            response_data = {
                'id': current_debugtalk.id,
                'code': merged_code,
                'project': current_debugtalk.project.id
            }
