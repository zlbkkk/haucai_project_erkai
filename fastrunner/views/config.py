...
class ConfigView(GenericViewSet):
    ...
    def list(self, request):
        ...
        queryset = self.get_queryset().filter(project_filter).order_by("-update_time")
    ...
    def all(self, request, **kwargs):
        ...
        # 使用新的过滤条件，包含当前项目和公共项目的数据
        project_filter = get_project_filter_condition(pk)
        queryset = (
            self.get_queryset()
...
class HostIPView(GenericViewSet):
    ...
    def all(self, request, **kwargs):
        ...
        # 使用新的过滤条件，包含当前项目和公共项目的数据
        project_filter = get_project_filter_condition(pk)
        queryset = self.get_queryset().filter(project_filter).order_by("-update_time").values("id", "name")
