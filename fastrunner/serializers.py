...
class CaseStepSerializer(serializers.ModelSerializer):
    ...
    def get_body(self, obj):
        try:
            # 安全地解析 body 数据
            body = eval(obj.body) if obj.body else {}
            if not isinstance(body, dict):
                body = {}
        except (SyntaxError, ValueError, TypeError) as e:
            logger.warning(f"Failed to parse CaseStep body for CaseStep {obj.id}: {e}")
            body = {}

        # 检查是否为配置类型
        if body.get("request", {}).get("base_url"):
            return {"name": body.get("name", ""), "method": "config"}
        else:
            parse = Parse(body)
            parse.parse_http()
...
class APISerializer(serializers.ModelSerializer):
    ...
    def get_body(self, obj):
        try:
            # 安全地解析 body 数据
            body_data = eval(obj.body) if obj.body else {}
            if not isinstance(body_data, dict):
                body_data = {}
        except (SyntaxError, ValueError, TypeError) as e:
            logger.warning(f"Failed to parse API body for API {obj.id}: {e}")
            body_data = {}
    ...
    def get_relation_name(self, obj):
        try:
            ...
            # 安全地解析 tree 数据
            tree_data = eval(relation_obj.tree) if relation_obj.tree else []
            if not isinstance(tree_data, list):
                tree_data = []
            label = get_tree_relation_name(tree_data, obj.relation)
            return label
        except (models.Relation.DoesNotExist, SyntaxError, ValueError, TypeError) as e:
            logger.warning(f"Failed to get relation name for API {obj.id}: {e}")
            return ""
...
class ConfigSerializer(serializers.ModelSerializer):
    ...
    def get_body(self, obj):
        try:
            ...
            logger.warning(f"Failed to parse Config body for Config {obj.id}: {e}")
            body_data = {}

        parse = Parse(body_data, level="config")
        parse.parse_http()
        return parse.testcase
